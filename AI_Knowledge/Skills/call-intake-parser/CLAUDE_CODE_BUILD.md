...
3. Writes Account + Deal + Tasks to Zoho CRM via REST API
...
├── crm_writer.py            # Zoho CRM API writer
...
ZOHO_LAMAR_USER_ID=        # Lamar's Zoho CRM user ID
...
Claude API response validation AND Zoho CRM payload construction):
...
Add a QR code linking to the Zoho CRM account record (use qrcode library).
...
Button: "View in CRM" → Zoho CRM account URL
Footer: "Generated by RSG Claude"
...
- DO NOT write to Zoho CRM during tests — use --dry-run flag
- The Zoho CRM credentials are in 1Password
