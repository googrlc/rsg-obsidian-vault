...
### 2. Zoho CRM Record Formatter
Given raw information (from a call, email, or document), format it as a ready-to-paste Zoho CRM record with:
- All required fields filled
- Stage set correctly (matching Zoho CRM's exact picklist values)
- LOB identified
...
- Is the stage value using the exact picklist string from Zoho CRM?

**Known stage values (use exactly):**
- `Won - Bound`
- `In Progress`
- `Quoted`
- `Lost`
- `Follow Up`
(Confirm current list with Lamar if unsure — wrong strings create duplicate picklist options)
...
- **Zoho CRM:** Pipeline management. New business, renewals, leads all tracked here.
- **Sync:** NowCerts → Zoho CRM sync runs via automation. Manual entry in Zoho CRM may be overwritten if it conflicts with NowCerts data.
- **Gretchen's access:** NowCerts (confirmed), Zoho CRM (confirmed), carrier portals (varies)

---

## EXAMPLE TRIGGERS
- "Help me enter this new client in NowCerts"
- "Format this info for Zoho CRM"
