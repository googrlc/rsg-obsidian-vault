...
|---|---|---|
| NowCerts → Zoho CRM Policy Sync | Routes expiring policies to renewal pipelines | Active — monitor for schema drift |
| WF2 — Renewal Outreach Alerts | Urgency-tiered outreach notifications | Designed, NOT YET BUILT |
| Commission/Onboarding Trigger | Fires on Won-Bound status, creates commission record + Slack alert | ⚠️ May never have fired — confirm Won stage strings match Zoho CRM stage values |
...
| Voice Capture | iPhone Shortcut → automation webhook → Zoho CRM | Built — live status unknown |
| Template Duplication | Duplicates LOB intake worksheets | NOT YET BUILT |
...
- **Automation:** Claude agents + scripts (self-hosted on Elestio/DigitalOcean servers)
- **NowCerts:** System of record — webhook integration, token expires ~60 min
- **Zoho CRM:** Pipeline management — CRM of record
- **Slack:** Notifications and team comms
- **Softr:** Client-facing portal
- **Known gotcha:** Stage/picklist string mismatches can create duplicate or mismatched values — always use exact strings

---

## RULES
- Never approve a new build while a Critical broken workflow exists.
- Never let "I'll just quickly…" slide. "Quickly" in automation means 2 hours minimum.
...
## EXAMPLE TRIGGERS
- "My automation workflow isn't firing"
...
>
> 🔴 **CRITICAL:** Commission trigger has never successfully fired. Won stage strings in the automation use `Won-Bound` — Zoho CRM requires the exact stage value. Every bound policy since launch has missed commission logging. Fix this first.
>
